...
impl JsonRpcClient {
    ...
    pub async fn call(&mut self, method: &str, params: Value) -> Result<Value, String> {
        ...
        let stdin = self.child.stdin.as_mut().ok_or("No stdin for JSON-RPC server")?;
        stdin.write_all(request_str.as_bytes()).await.map_err(|e| e.to_string())?;
        stdin.flush().await.map_err(|e| e.to_string())?;
        
        let stdout = self.child.stdout.as_mut().ok_or("No stdout for JSON-RPC server")?;
