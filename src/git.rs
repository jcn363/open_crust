        ...
        Ok(status) if status.success() => {
            ...
            match commit_output {
                Ok(output) => {
                    ...
                    } else if stdout.contains("nothing to commit") || stdout.contains("working tree clean") {
...
pub fn undo() -> Result<String, String> {
    ...
    match output {
        Ok(out) if out.status.success() => Ok("Successfully undid the last checkpoint.".to_string()),
        Ok(out) => Err(format!("Failed to undo: {}", String::from_utf8_lossy(&out.stderr))),
